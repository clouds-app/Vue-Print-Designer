...
import type { DesignerStore, PrepareEnvironmentFn, RenderContent } from './types';
...
export const createImageRenderer = (deps: ImageRendererDeps) => {
  const { store, prepareEnvironment, resolveRenderSource, handleTablePagination, updatePageNumbers } =
    deps;
  ...
  return { getPrintHtml, processContentForImage, generatePageImages, createPdfDocument };
};
