...
import type { DesignerStore, PrepareEnvironmentFn, CreateRepeatedPagesFn } from './types';
...
export const createRenderEngine = (deps: { 
  ...
  const { resolveRenderSource } = createIframeRenderer({ store, createRepeatedPages });
  const { updatePageNumbers, handleTablePagination } = createPagination({ store });
  const { 
    getPrintHtml, 
